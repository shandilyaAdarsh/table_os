...
import { ROLE_HIERARCHY, type Role } from '../../../types/rbac.types';
...
export async function assignRole(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const body     = validate(AssignRoleSchema, req.body);
    ...
    res.status(200).json(formatSuccess(null, 'Role assigned successfully'));
  } catch (err) {
    next(err);
  }
}
...
export async function revokeMembership(
  ...
  try {
    const tenantId = String(req.params.tenantId);
    const body     = validate(RevokeSchema, req.body);

    await revokeTenantMembership(body.userId, tenantId, req.context.id);

    res.status(200).json(formatSuccess(null, 'Membership revoked'));
  } catch (err) {
    next(err);
  }
}
...
export async function getUserBranchAccesses(
  ...
  try {
    ...
    res.status(200).json(formatSuccess(access));
  } catch (err) {
    next(err);
  }
}
...
export async function getActiveSessions(
  ...
  try {
    ...
    res.status(200).json(formatSuccess(sessions));
  } catch (err) {
    next(err);
  }
}
