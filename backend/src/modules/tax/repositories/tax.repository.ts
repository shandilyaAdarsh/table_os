...
// OCC error code used across all update/delete flows.
// Supabase returns PGRST116 (single() with no matching row) when the
// WHERE version_num = ? clause finds no row — i.e. a stale update.
...
export class TaxRepository {
  ...
  async updateProfile(
    ...
    const { data, error } = await this.supabase
      ...
      .update({
        ...payload,
        updated_by:  userId,
        // OCC: atomically increment version_num; row must match current value
        version_num: payload.version_num + 1,
      })
      .eq('tenant_id',   tenantId)
      .eq('id',          id)
      .eq('version_num', payload.version_num)   // stale check
      .is('deleted_at',  null)
      .select()
      .single();

    if (error) {
      // PGRST116 = no row matched → version_num mismatch (stale OCC)
  ...
  async softDeleteProfile(
    ...
    const { count, error } = await this.supabase
      ...
      .update({
        ...
        updated_by:  userId,
        // OCC: atomically increment version_num
        version_num: versionNum + 1,
      })
      .eq('tenant_id',   tenantId)
      .eq('id',          id)
      .eq('version_num', versionNum)            // stale check
      .is('deleted_at',  null);

    if (error) {
      throw new AppError('Failed to soft delete tax profile', 500, ErrorCode.INTERNAL_SERVER_ERROR, true, { error });
    }
    // count === 0 → the WHERE version_num clause matched nothing → stale OCC
    if (count === 0) {
      throw new AppError(OCC_CONFLICT_MSG, 409, ErrorCode.CONFLICT);
    }
  }
  ...
  async createRate(tenantId: string, userId: string, payload: CreateTaxRateDTO): Promise<TaxRate> {
    ...
    if (error) {
      if (error.code === '23P01') {
        // Exclusion constraint: this profile already has an active rate
        // overlapping the requested effective window.
        throw new AppError(
          'An active tax rate already exists for this profile within the requested effective window. ' +
          'Deactivate the existing rate before creating a new one.',
          409,
          ErrorCode.CONFLICT,
          true,
          { error },
  ...
  async deactivateRate(
    ...
    // count === 0 → version_num mismatch or row already deleted → stale OCC
    if (count === 0) {
      throw new AppError(OCC_CONFLICT_MSG, 409, ErrorCode.CONFLICT);
    }
  }
  ...
  async assignMenuItemProfile(
    tenantId: string,
    userId: string,
    payload: AssignMenuItemTaxProfileDTO,
  ): Promise<MenuItemTaxProfile> {
    // Soft-delete any existing active mapping for this item before creating the new one.
    // This preserves historical assignment records for auditing (append-friendly).
    await this.supabase
  ...
  async resolveTax(
    ...
    menuItemId: string,
    effectiveAt: string,
  ): Promise<ResolvedTaxRPC | null> {
    const { data, error } = await this.supabase.rpc('resolve_tax_for_menu_item', {
  ...
  async resolveBatchTax(
    tenantId: string,
    menuItemIds: string[],
    effectiveAt: string,
  ): Promise<ResolvedTaxBatchRPC[]> {
    const { data, error } = await this.supabase.rpc('resolve_tax_for_menu_items_batch', {
      p_tenant_id:      tenantId,
