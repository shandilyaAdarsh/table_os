...
class ReplayRecoveryCoordinator {
  ...
  Future<void> executeRecovery({
    ...
    debugPrint('[ReplayRecoveryCoordinator] Starting replay recovery for branch: $branchId. Last known seq: $lastKnownSequence');
    ...
    debugPrint('[ReplayRecoveryCoordinator] Recovered delta events applied to projection store');

    // 2. Trigger full UI projection rebuild from the now-authoritative store
    await _rebuildEngine.rebuildAll();

    debugPrint('[ReplayRecoveryCoordinator] Replay recovery and full rebuild complete');
  }
}
