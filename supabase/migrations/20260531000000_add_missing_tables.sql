...
-- ============================================================================
-- CLEANUP: Drop tables if they exist from failed migration attempts
-- ============================================================================
DROP TABLE IF EXISTS public.restaurant_settings CASCADE;
DROP TABLE IF EXISTS public.menu_snapshots CASCADE;
DROP TABLE IF EXISTS public.guest_sessions CASCADE;
DROP TABLE IF EXISTS public.profiles CASCADE;
DROP TABLE IF EXISTS public.credential_invites CASCADE;
...
-- RLS Policy: Users in same tenant can manage (simplified - no role check due to enum constraints)
CREATE POLICY "users_tenant_manage" ON public.users
  FOR ALL
  USING (auth_id = auth.uid());
...
-- RLS Policy: Users in same tenant can manage profiles (simplified - no role check due to enum constraints)
CREATE POLICY "profiles_tenant_manage" ON public.profiles
  FOR ALL
  USING (auth_id = auth.uid());
